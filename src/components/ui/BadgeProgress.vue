...
export default {
  ...
  computed: {
    ...
    getProgress() {
      const activitiesDone = this.task.activities.filter(activity => activity.complete === true);
      const done = activitiesDone.length === undefined ? 1 : activitiesDone.length;
      const total = this.task.activities.length === undefined ? 1 : (this.task.activities.length - 1);
